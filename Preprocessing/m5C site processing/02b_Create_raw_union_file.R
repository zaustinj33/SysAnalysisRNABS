...
mRNA_dfs <- lapply(mRNAfiles, function(x) {
  "\code: Aggregates sample files into a compelete m5C matrix. Can use either suffix depending on
  stage of filtering.
  @param mRNAfiles: a list of meRanCall files. Can change suffix to match extension.
  @return: mergable meRanCall file with only important columns
