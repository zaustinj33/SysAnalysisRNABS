...
cg=sns.clustermap(result_df.reset_index(drop=True), row_linkage=row_linkage, col_linkage=col_linkage, cmap=custom_color_map,
...
def BH_test(set1, set2):
    ...
    p_adj = []
    try:
        len(count_df[set1]) == len(cov_df[set1])
    except:
    ...
    #master_set.to_csv('Dotplot_'+ set1 + set2 + '_table.csv')

    return master_set

test_BH = pd.DataFrame(BH_test('G2','G3'))
#%%
...
sns.scatterplot(data=test_BH, x='ML_1', y='ML_2', style = 'shape',
                markers=markers, s=25)
plt.xlim(0,1)
plt.ylim(0,1)
